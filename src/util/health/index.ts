...
export class HealthChecker {
  ...
  async checkHealth(): Promise<HealthStatus> {
    ...
    const [database, rabbitmq, spaceInvadersAPI, diskPersistence] = await Promise.allSettled([
      this.checkDatabase(),
      this.checkRabbitMQ(),
    ...
    const health: HealthStatus = {
      ...
      metrics: {
        ...
        failedFlashesCount: diskPersistence_instance.getFailedFlashesCount()
      }
    };

    const checkDuration = Date.now() - startTime;
    console.log(`[HealthChecker] Health check completed in ${checkDuration}ms - Status: ${overallStatus}`);
  ...
  private async checkDatabase(): Promise<ServiceHealth> {
    ...
    try {
      ...
      const result = await this.dbPool.query('SELECT COUNT(*) as count FROM flashes LIMIT 1');
  ...
  private async checkRabbitMQ(): Promise<ServiceHealth> {
    ...
    try {
      ...
      await channel.checkQueue(process.env.RABBITMQ_QUEUE || 'test_queue');
  ...
  private async checkSpaceInvadersAPI(): Promise<ServiceHealth> {
    ...
    try {
      ...
      const response = await axios.get(`${this.apiUrl}/flashinvaders/flashes/`, {
        timeout: 5000,
      ...
      const isDegraded = (error as any).code === 'ECONNABORTED' || 
