...
async function performanceCheck(): Promise<void> {
    ...
    try {
        ...
        const healthChecker = new HealthChecker(pool, config.rabbitmq.url);
        ...
        Object.entries(health.services).forEach(([service, status]) => {
            ...
            const responseTime = status.responseTime
                ...
                : "";
            console.log(
                `  ${emoji} ${service}: ${status.status}${responseTime}`,
            );
            if (status.error) {
        ...
        if (performanceReport.tableStats) {
            ...
            console.log(
                `Live Tuples: ${stats.n_live_tup?.toLocaleString() || "N/A"}`,
            );
            console.log(
                `Tuples Returned: ${stats.n_tup_ret?.toLocaleString() || "N/A"}`,
            );
        }
        ...
        if (performanceReport.indexStats.length > 0) {
            performanceReport.indexStats.forEach((index) => {
                console.log(
                    `${index.indexname}: ${index.hit_ratio_percent}% hit ratio`,
                );
                console.log(
                    `  Blocks Read: ${index.idx_blks_read?.toLocaleString() || "0"}`,
                );
                console.log(
                    `  Blocks Hit: ${index.idx_blks_hit?.toLocaleString() || "0"}`,
                );
            });
        } else {
        ...
        if (performanceReport.slowQueries.length > 0) {
            performanceReport.slowQueries.slice(0, 5).forEach((query, i) => {
                console.log(
                    `${i + 1}. Avg Time: ${Math.round(query.mean_exec_time)}ms | Calls: ${query.calls}`,
                );
                console.log(`   Query: ${query.query.substring(0, 100)}...`);
                if (query.hit_percent !== null) {
                    console.log(
                        `   Cache Hit: ${Math.round(query.hit_percent)}%`,
                    );
                }
            });
        } else {
            console.log(
                "No slow query data available (pg_stat_statements not enabled)",
            );
        }
...
async function applyOptimizations(): Promise<void> {
    ...
    try {
        ...
        console.log(
            "3. Consider enabling pg_stat_statements for query analysis",
        );
    } catch (error) {
...
function generateRecommendations(
    ...
    if (health.status !== "healthy") {
        const unhealthyServices = Object.entries(health.services)
            .filter(
                ([_, status]: [string, any]) => status.status === "unhealthy",
            )
            .map(([service]) => service);
    ...
    if (health.metrics.failedFlashesCount > 5) {
        recommendations.push(
            "Clear failed flashes or investigate recurring failures",
        );
    }
    ...
    if (heapUsedMB > 512) {
        recommendations.push(
            "Consider increasing GC frequency or optimizing memory usage",
        );
    }
    ...
    if (dbPerf.indexStats.length === 0) {
        recommendations.push(
            "Apply database performance indexes using --optimize flag",
        );
    } else {
        const lowHitRatios = dbPerf.indexStats.filter(
            (idx: any) => idx.hit_ratio_percent < 95,
        );
        if (lowHitRatios.length > 0) {
            recommendations.push(
                "Some indexes have low hit ratios - monitor query patterns",
            );
        }
    }
    ...
    if (dbPerf.slowQueries.length > 0) {
        ...
        if (avgSlowTime > 1000) {
            recommendations.push(
                "Optimize slow queries or add targeted indexes",
            );
        }
    }

    // Service-specific recommendations
    Object.entries(health.services).forEach(
        ([service, status]: [string, any]) => {
            if (status.responseTime && status.responseTime > 2000) {
                recommendations.push(
                    `${service} response time is high (${status.responseTime}ms) - investigate`,
                );
            }
        },
    );
